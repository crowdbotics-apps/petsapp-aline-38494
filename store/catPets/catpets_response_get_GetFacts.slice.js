...
const catpets_response_get_GetFactsSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      .addCase(catpets_get_fact_read.pending, (state, action) => {
        if (state.api.loading === "idle") {
          state.api.loading = "pending"
        }
      })
      .addCase(catpets_get_fact_read.fulfilled, (state, action) => {
      ...
      .addCase(catpets_get_fact_read.rejected, (state, action) => {
        if (state.api.loading === "pending") {
